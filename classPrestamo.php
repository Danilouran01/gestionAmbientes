...
class Prestamo extends Conexion
{
    ...
    public function obtenerPrestamosAmbientePorFechaEstado($fecha_inicial,$fecha_final,$estado)
    {


        $this->conectar();

        $prestamos_fecha_estado = mysqli_prepare($this->con, "SELECT `id_prestamo`,`nombre`,`apellido`, `fecha_prestamo`, `hora_prestamo`, `fecha_entrega`, `hora_entrega`, `observaciones`, `id_numero_ambiente`, prestamo.numero_documento, `estado_prestamo` FROM usuario INNER JOIN `prestamo` ON usuario.numero_documento=prestamo.numero_documento WHERE id_numero_ambiente IS NOT NULL AND (estado_prestamo= ? AND (fecha_prestamo BETWEEN ? and ? ) ) ORDER BY prestamo.id_prestamo DESC;");
        $prestamos_fecha_estado->bind_param('sss',$estado,$fecha_inicial,$fecha_final);
        $prestamos_fecha_estado->execute();
        $resultado_consulta =  $prestamos_fecha_estado->get_result();
        ...
        return $resultado_consulta->fetch_all(MYSQLI_ASSOC);

       
    }

    public function filtrarPrestamosAmbientePorFechaOPorEstado($fecha_inicial,$fecha_final,$estado)
    {

        $this->conectar();

        $prestamos_fecha = mysqli_prepare($this->con, "SELECT `id_prestamo`,`nombre`,`apellido`, `fecha_prestamo`, `hora_prestamo`, `fecha_entrega`, `hora_entrega`, `observaciones`, `id_numero_ambiente`, prestamo.numero_documento, `estado_prestamo` FROM usuario INNER JOIN `prestamo` ON usuario.numero_documento=prestamo.numero_documento WHERE id_numero_ambiente IS NOT NULL AND (prestamo.estado_prestamo= ? OR fecha_prestamo BETWEEN ? AND ?) ORDER BY prestamo.id_prestamo DESC;");
        $prestamos_fecha->bind_param('sss',$estado,$fecha_inicial,$fecha_final);
        $prestamos_fecha->execute();
        $resultado_sql =  $prestamos_fecha->get_result();
        ...
        mysqli_close($this->con);
        return $resultado_sql->fetch_all(MYSQLI_ASSOC);
...
public function filtrarPrestamosElementosPorFechaOPorEstado($fecha_inicial,$fecha_final,$estado){
    $this->conectar();
    $prestamo_elemento_fecha= mysqli_prepare($this->con,"SELECT prestamo.id_prestamo, usuario.numero_documento,usuario.nombre,usuario.apellido,COUNT(prestamo.id_prestamo) as cant,fecha_prestamo,hora_prestamo,fecha_entrega,hora_entrega,observaciones,estado_prestamo FROM usuario INNER join prestamo on usuario.numero_documento =prestamo.numero_documento INNER JOIN detalle_prestamo on prestamo.id_prestamo =detalle_prestamo.id_prestamo INNER join elementos on elementos.serial =detalle_prestamo.serial INNER JOIN tipo_dispositivo on tipo_dispositivo.id_tipo_dispositivo=elementos.tipo_dispositivo WHERE prestamo.estado_prestamo=? OR fecha_prestamo BETWEEN ? AND ? GROUP BY prestamo.id_prestamo ORDER BY prestamo.id_prestamo DESC;");
    $prestamo_elemento_fecha->bind_param('sss',$estado,$fecha_inicial,$fecha_final);
    $prestamo_elemento_fecha->execute();

    
    $resultado_fecha =  $prestamo_elemento_fecha->get_result();
    ...
    mysqli_close($this->con);
    return $resultado_fecha->fetch_all(MYSQLI_ASSOC);
...
public function filtrarPrestamosElementosPorFechaEstado($fecha_inicial,$fecha_final,$estado){
    $this->conectar();
    $prestamo_elemento= mysqli_prepare($this->con,"SELECT prestamo.id_prestamo, usuario.numero_documento,usuario.nombre,usuario.apellido,COUNT(prestamo.id_prestamo) as cant,fecha_prestamo,hora_prestamo,fecha_entrega,hora_entrega,observaciones,estado_prestamo FROM usuario INNER join prestamo on usuario.numero_documento =prestamo.numero_documento INNER JOIN detalle_prestamo on prestamo.id_prestamo =detalle_prestamo.id_prestamo INNER join elementos on elementos.serial =detalle_prestamo.serial INNER JOIN tipo_dispositivo on tipo_dispositivo.id_tipo_dispositivo=elementos.tipo_dispositivo WHERE prestamo.estado_prestamo=? AND fecha_prestamo BETWEEN ? AND ? GROUP BY prestamo.id_prestamo ORDER BY prestamo.id_prestamo DESC;");
    $prestamo_elemento->bind_param('sss',$estado,$fecha_inicial,$fecha_final);
    $prestamo_elemento->execute();

  
    $resultado_sql =  $prestamo_elemento->get_result();
