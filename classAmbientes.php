...
class Ambientes extends conexion
{
    ...
    public function registrarAmbiente()
    {
        ...
        $pre = mysqli_prepare($this->con, "INSERT INTO `ambientes`(`id_numero_ambiente`, `piso`, `linea_formacion`, `estado`, `cantidad_sillas`,`mesas`) VALUES (?,?,?,?,?,?)");
        $pre->bind_param("iiiiii", $this->id_ambiente, $this->piso,$this->linea_formacion, $this->estado,$this->sillas,$this->mesas);
        $pre->execute();
        if ($pre) {
    ...
    {
        $this->conectar();
        $sql = "SELECT *  FROM `ambientes` inner JOIN estado_ambiente ON ambientes.estado=estado_ambiente.id_estado_ambiente INNER JOIN linea_formacion on linea_formacion.id_linea=ambientes.linea_formacion WHERE id_numero_ambiente='$id' ";
        $resultado = $this->con->query($sql);
        return $resultado;
        $this->con->close();
    }
